...
Alpine.data("ChallengeBoard", () => ({
  ...
  initSvgPaths() {
    ...
    console.log("Number of paths:", paths.length); // Cuántos paths tiene tu SVG
    console.log("Number of challenges:", this.challenges.length); // Cuántos retos están cargados

    paths.forEach((path, index) => {
      if (index < this.challenges.length) {
        console.log(
          `Setting data-challenge-id to ${this.challenges[index].id} for path ${index}`
        );
        path.setAttribute("data-challenge-id", this.challenges[index].id);
      }
    });
  },
